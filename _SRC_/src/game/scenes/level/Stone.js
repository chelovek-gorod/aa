...
export default class Stone extends Sprite {
    ...
    tick(deltaMs) {
        if (this.isFalling) {
            this.updateFalling(deltaMs)
        } else {
            this.updateArc(deltaMs)
        }
    }
    ...
    updateArc(deltaMs) {
        ...
        // Физика
        this.velocity.y += GRAVITY * scaledDeltaMs
        this.x += this.velocity.x * scaledDeltaMs
        this.y += this.velocity.y * scaledDeltaMs
        
        // Вращение
        this.rotation += this.rotationSpeed * scaledDeltaMs
    ...
    updateFalling(deltaMs) {
        ...
        // Замедление вращения
        this.rotationSpeed *= Math.pow(DAMPING_PER_MS, scaledDeltaMs)
        this.rotation += this.rotationSpeed * scaledDeltaMs
        
        // Исчезновение
        this.alpha -= FADE_SPEED * scaledDeltaMs
